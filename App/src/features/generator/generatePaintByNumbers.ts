...
export async function generatePaintByNumbers(
  ...
  await ColorReducer.applyKMeansClustering(imageData, kmeansOutput, vendorSettings, (kmeans) => {
    ...
    report('kmeans', local, `Farben werden gruppiert (${kmeans.currentDeltaDistanceDifference.toFixed(2)} Delta).`);
  });
  ...
  report('colorMap', 1, `${colorMapResult.colorsByIndex.length} Farben erkannt.`);
