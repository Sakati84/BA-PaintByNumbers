export type FreshVectorRenderConfig = {
  fillMode: 'color' | 'white';
  boundaryMode: 'none' | 'black' | 'color';
  markerMode: 'none' | 'circles';
};
...
function buildFillPaths(labelMap: Uint8Array, paletteRgb: Float32Array, width: number, height: number): string[] {
  const commandsByLabel = Array.from({ length: paletteRgb.length / 3 }, () => [] as string[]);
  for (let y = 0; y < height; y += 1) {
    ...
    while (x < width) {
      ...
      const startX = x;
      x += 1;
      while (x < width && labelMap[rowOffset + x] === label) {
        x += 1;
      }
      if (label < commandsByLabel.length) {
        const runLength = x - startX;
        commandsByLabel[label].push(`M${startX} ${y}h${runLength}v1h-${runLength}z`);
      }
    }
  }

  const paths: string[] = [];
  for (let label = 0; label < commandsByLabel.length; label += 1) {
    const commands = commandsByLabel[label];
    if (commands.length > 0) {
      paths.push(`<path d="${commands.join('')}" fill="${svgRgb(paletteColor(paletteRgb, label))}"/>`);
    }
  }
  return paths;
}
...
export function renderFreshVectorSvg(
  ...
  if (config.fillMode === 'white') {
    ...
    parts.push(...buildFillPaths(labelMap, paletteRgb, width, height));
  }
  ...
  if (config.markerMode === 'circles') {
    for (const placement of placements) {
      ...
      parts.push(
        `<circle cx="${placement.x.toFixed(2)}" cy="${placement.y.toFixed(2)}" r="${placement.radius.toFixed(2)}" fill="${svgRgb(paletteColor(paletteRgb, placement.colorIndex))}" stroke="${svgRgb(OUTLINE)}" stroke-width="${strokeWidth.toFixed(2)}"/>`,
      );
    }
  }
  parts.push('</svg>');
