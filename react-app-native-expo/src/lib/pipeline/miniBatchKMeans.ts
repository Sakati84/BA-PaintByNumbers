...
function copySample(samples: Float32Array, sampleIndex: number, dimensions: number, target: Float32Array, targetOffset: number): void {
...
export function quantizeByCenters(
  labSamples: Float32Array,
  centers: Float32Array,
): { labels: Uint16Array | Uint32Array; quantizedLab: Float32Array } {
  const labels = assignNearestCenters(labSamples, centers, 3);
  const quantizedLab = new Float32Array(labSamples.length);
  for (let sampleIndex = 0; sampleIndex < labels.length; sampleIndex += 1) {
    ...
    const centerOffset = label * 3;
    quantizedLab[sourceOffset] = centers[centerOffset];
    quantizedLab[sourceOffset + 1] = centers[centerOffset + 1];
    quantizedLab[sourceOffset + 2] = centers[centerOffset + 2];
  }
  return { labels, quantizedLab };
}
