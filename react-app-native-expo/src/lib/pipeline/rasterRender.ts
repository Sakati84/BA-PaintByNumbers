...
function measureBitmapText(text: string, scale: number): { width: number; height: number } {
  ...
  const spacing = scale;
  return {
    width: text.length * glyphWidth + (text.length - 1) * spacing,
    height: 5 * scale,
  };
}
...
export function renderBrightColorCirclesTemplate(args: {
  ...
  const base = renderRgbaBufferFromLabelMap(
    args.labelMap,
    brightPaletteRgb,
    args.width,
    args.height,
  ...
  applyOutlinesToPixels(pixels, args.boundaryMask, args.width, args.height);
...
export function renderColorCirclesTemplate(args: {
  ...
  for (const region of [...args.regions].sort((left, right) => right.area - left.area || left.regionId - right.regionId)) {
    ...
    const circleRadius = Math.max(
      ...
      Math.round(textSize.width * 0.7 + 2),
    );
